...
> **📓 About "Try it in Python" boxes:** Throughout this lesson, these boxes reference specific code examples by ID — e.g. **W11-CS03** means *Week 11, Code Snippet 3*. Find the matching code under **Notes → Python Code Snippets** for this week; each entry there is labelled with its ID.
...
- **Motor behavior:** Repetitive movements like walking, breathing, or—for example, in repetitive limb motion—moving a pencil back and forth
...
> **📓 Try it in Python**
>
> Get set up for trigonometry and convert between angle units:
> - **W11-CS01** — *Trig Imports & Setup*: Bring in `numpy`, plotting tools.
> - **W11-CS02** — *Degrees ↔ Radians*: Convert angles in either direction.
>
...
> **📓 Try it in Python**
>
> Verify the special-angle table programmatically:
> - **W11-CS03** — *Exact Values Table*: Tabulate sin, cos, tan at key angles.
>
> Find these under **Notes → Python Code Snippets** for this week.
...
**Example:** For $x(t) = \frac{7}{2}\cos(\pi t)$:
- Period: $T = \frac{2\pi}{\pi} = 2$ seconds
- Frequency: $f = \frac{1}{2} = 0.5$ Hz

> **📓 Try it in Python**
>
> Plot sinusoidal functions and explore their parameters:
> - **W11-CS04** — *Sine and Cosine Graphs*: Basic plots over one period.
> - **W11-CS05** — *Effect of A, B, C, D*: Visualise amplitude, period, phase, vertical shift.
>
> Find these under **Notes → Python Code Snippets** for this week.
...
### 5.3 Worked Example: Pencil Movement
...
> **📓 Try it in Python**
>
> Fit sinusoidal models and explore real applications:
> - **W11-CS06** — *Fitting Sinusoidal Models*: Recover $A, B, C, D$ from data.
> - **W11-CS07** — *Pencil Movement Example*: Animate simple harmonic motion.
> - **W11-CS10** — *Interactive Calculator*: Tweak parameters and see the response.
>
> Find these under **Notes → Python Code Snippets** for this week.
...
> **📓 Try it in Python**
>
> Model real cyclic phenomena:
> - **W11-CS08** — *Circadian Body Temperature*: Daily temperature rhythm.
> - **W11-CS09** — *Northern vs Southern Hemisphere*: Seasonal sinusoidal comparison.
>
> Find these under **Notes → Python Code Snippets** for this week.
...
Sine and cosine are the **same wave, shifted by $\tfrac{\pi}{2}$**:

$$\sin\theta = \cos\!\left(\theta - \frac{\pi}{2}\right) \qquad \cos\theta = \sin\!\left(\theta + \frac{\pi}{2}\right)$$

These are called **co-function identities** because $\sin$ and $\cos$ are co-functions of each other.

**Converting the phase shift when switching between sin and cos:**

When rewriting $A\sin(B(t-C))+D$ as an equivalent cosine model (or vice versa), the amplitude $A$, frequency $B$, and vertical shift $D$ are unchanged — only the phase shift $C$ adjusts:

| Conversion      | New phase shift $C'$       |
| --------------- | -------------------------- |
| $\sin \to \cos$ | $C' = C + \dfrac{\pi}{2B}$ |
| $\cos \to \sin$ | $C' = C - \dfrac{\pi}{2B}$ |

**Example:** Rewrite $2\sin\!\left(\pi t - \tfrac{\pi}{3}\right)$ as a cosine model.

Here $B = \pi$ and $C = \tfrac{1}{3}$ (since $B(t-C) = \pi(t - \tfrac{1}{3})$).

$$C' = \frac{1}{3} + \frac{\pi}{2\pi} = \frac{1}{3} + \frac{1}{2} = \frac{5}{6}$$

So $2\sin\!\left(\pi t - \tfrac{\pi}{3}\right) = 2\cos\!\left(\pi t - \tfrac{5\pi}{6}\right)$.
...
| Co-function identities   | $\sin\theta = \cos(\theta - \tfrac{\pi}{2})$, $\cos\theta = \sin(\theta + \tfrac{\pi}{2})$ |
| Sin→Cos phase conversion | $C' = C + \dfrac{\pi}{2B}$                                                                 |
| Cos→Sin phase conversion | $C' = C - \dfrac{\pi}{2B}$                                                                 |
