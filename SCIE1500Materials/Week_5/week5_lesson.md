...
> **📓 About "Try it in Python" boxes:** Throughout this lesson, these boxes reference specific code examples by ID — e.g. **W5-CS03** means *Week 5, Code Snippet 3*. Find the matching code under **Notes → Python Code Snippets** for this week; each entry there is labelled with its ID.
...
Finding MEY requires derivatives of more complex expressions: products, quotients, and compositions. That is the technical focus of this week.
...
> **📓 Try it in Python**
>
> - **W5-CS01** — *Setting Up SymPy for Calculus*: Run this first.
> - **W5-CS02** — *Basic Differentiation*: SymPy `diff()` for polynomials.
> - **W5-CS03** — *Product Rule Verification*: Confirm $(fg)' = f'g + fg'$ symbolically.
> - **W5-CS04** — *Chain Rule with Composite Functions*: Differentiate $f(g(x))$ step-by-step.
>
...
**Why $e$ is special:** Since $\ln(e) = 1$, the derivative of $e^x$ is simply $e^x$, with no extra constant factor. This is why $e$ is called the **natural base** for exponential functions.
...
**Example 5.10:** Differentiate $y = \frac{3}{4}x^4 + \frac{1}{4}x^2 - 3x + \ln(x)$.
...
### 4.3 Local vs Global Optima and Inflection Points

**A crucial distinction:** The second derivative test finds *local* optima, but the best outcome in a real problem may require the *global* optimum.
...
**Practical rule for closed intervals:** On $[a, b]$, evaluate $f$ at every critical point **and** at the endpoints $a$ and $b$. The largest value is the global maximum; the smallest is the global minimum.

#### Inflection Points

An **inflection point** is where the curve changes from concave up to concave down (or vice versa).

> **At an inflection point:** $f''(x) = 0$ **and** the sign of $f''$ changes.

**Important warning:** $f'(x) = 0$ and $f''(x) = 0$ does **not** guarantee a maximum or minimum. It may be an inflection point.

**Example:** $f(x) = x^3$ at $x = 0$.
- $f'(0) = 0$ and $f''(0) = 0$
- But $f''$ changes sign at $x = 0$ (negative → positive)
- So $x = 0$ is an **inflection point**, not an extremum

Inflection points correspond to the steepest rate of change-important in population growth models where the curve switches from accelerating to decelerating growth.

---

### 4.4 Gradient Descent and Newton's Method

When we cannot solve $f'(x) = 0$ algebraically, we use iterative numerical methods.

#### Gradient Descent

Imagine standing on a hilly landscape in fog. You can't see the bottom, but you can feel the slope underfoot. Strategy: take a small step *downhill* (opposite to the gradient) and repeat.

$$x_{n+1} = x_n - \alpha \cdot f'(x_n)$$

where $\alpha > 0$ is the **step size** (learning rate). This is the same calculus used inside modern machine learning.

#### Newton's Method

Gradient descent can be slow. Newton's method uses the **second derivative** for smarter, faster steps:

$$x_{n+1} = x_n - \frac{f'(x_n)}{f''(x_n)}$$

**Simple example:** Find the minimum of $f(x) = x^2 - 4x + 5$.
- $f'(x) = 2x - 4$, $\quad f''(x) = 2$
- From $x_0 = 0$: $x_1 = 0 - \frac{-4}{2} = 2$ (exact in 1 step!)

|               | Gradient Descent    | Newton's Method       |
| ------------- | ------------------- | --------------------- |
| Uses          | $f'$ only           | $f'$ and $f''$        |
| Convergence   | Linear (many steps) | Quadratic (few steps) |
| Cost per step | Low                 | Higher                |

**Caution:** Newton's method converges to the *nearest* local minimum, not necessarily the global one. Starting-point choice matters for functions with multiple local optima.
...
> **📓 Try it in Python**
>
> - **W5-CS05** — *Solving $f'(x) = 0$ for Critical Points*: Locate optima symbolically.
> - **W5-CS06** — *Second Derivative Test*: Classify each critical point as max, min, or inflection.
> - **W5-CS11** — *Plotting Functions and Derivatives*: See $f$, $f'$, and $f''$ on one figure.
>
> Find these under **Notes → Python Code Snippets** for this week.
...
**Key insight:** MEY maintains a **higher stock** than MSY, reducing fishing cost and increasing profit despite slightly lower harvest.

### 5.4 From MEY to the Supply Curve

The MEY analysis does more than give the optimal stock for one price. It tells us how optimal harvest responds to *any* price $p$.

**Deriving the supply curve:** Setting $\pi'(S) = 0$ gives $S^*(p)$ as a function of price. Substituting back into the growth function:

$$H^*(p) = G(S^*(p)) = g \, S^*(p)\left(1 - \frac{S^*(p)}{K}\right)$$

This is the industry **supply curve**: how much fish is optimally supplied at each market price.

**Key result:** As price $p$ rises, optimal stock $S^*$ falls and harvest $H^*$ rises (up to a point). This yields the familiar upward-sloping supply curve, derived from biology and calculus.

**Student challenge - sonar shifts the supply curve:** Modern fishing vessels use sonar to locate fish schools, increasing the catchability coefficient $e$.

1. If $e$ increases, what happens to the cost of catching a given harvest?
2. Does the supply curve shift **left** or **right**?
3. What happens to MEY stock level $S^*$?
4. Is this good or bad for the fish population?

*(Hint: Higher $e$ means each boat catches more per trip → cost per tonne drops.)*

> **📓 Try it in Python**
>
> - **W5-CS07** — *Schaefer Model: Finding MSY*: Maximum Sustainable Yield via $dY/dE = 0$.
> - **W5-CS08** — *Bioeconomic Model: Finding MEY*: Maximum Economic Yield with cost included.
>
> Find these under **Notes → Python Code Snippets** for this week.

---

## 6. Application: Profit Maximization Word Problems
...
### 6.2 Worked Example: Livestock Fattening
...
> **📓 Try it in Python**
>
> - **W5-CS09** — *Constrained Optimization: Fencing Problem*: Classic max-area-with-fixed-perimeter setup.
> - **W5-CS10** — *Profit Maximization: Livestock Problem*: Walk-through
> - **W5-CS12** — *Parametric Optimization*: Optimum changes as a parameter sweeps.
>
> Find these under **Notes → Python Code Snippets** for this week.
...
| Global optima on $[a,b]$ | Compare critical points **and** endpoints $a$, $b$  |
| Inflection point         | $f''(x) = 0$ and sign of $f''$ changes              |
