...
![The Chain Rule](images/chain_rule.svg "The chain rule: derivative of outer times derivative of inner")
...
The function $e^x$ has the remarkable property of being **its own derivative**:

$$\frac{d}{dx}[e^x] = e^x$$

For a general exponential with a linear argument:
...
*Solution:* $y' = 5 \cdot (-0.2) \cdot e^{-0.2x} = -e^{-0.2x}$
...
![Finding Local Extrema](images/optimization.svg "Using the first derivative test to identify local maxima and minima")
