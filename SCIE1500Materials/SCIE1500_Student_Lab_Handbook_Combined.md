...
1. [Lab Structure and Expectations](#1-lab-structure-and-expectations)
2. [Assessment Overview](#2-assessment-overview)
3. [Group Presentations](#3-group-presentations)
4. [Problem Briefs: Weeks 1–12](#4-problem-briefs-weeks-012)
5. [Group Worksheet Template](#5-group-worksheet-template)
6. [Presentation Rubric](#6-presentation-rubric)
...
1. Rate **themselves** on their contribution (0–100%)
2. Rate **each teammate** on their contribution (0–100%)
...
![Global plastic production data with rate of change trends](handbook_images/W1_plastic_production.svg)
...
![Comparing exponential growth and decay across three domains](handbook_images/W2_doubling_times.svg)
...
1. **Plastic production:** Write an exponential model $P(t) = P_0 e^{kt}$ where $t$ is years since 1950.
   - Given: 8.4% annual growth rate
   - Find the value of $k$ (hint: $e^k = 1.084$)
   - Predict production in 2025

2. **Bacterial growth:** Write a model $N(t) = N_0 \cdot 2^{t/T}$ where $T$ is the doubling time in minutes.
   - How many bacteria after 2 hours?
   - Convert to the form $N(t) = N_0 e^{kt}$ and find $k$

3. **Radioactive decay:** Write a model $A(t) = A_0 e^{-\lambda t}$ where $t$ is years.
   - Find $\lambda$ using the half-life
   - What fraction remains after 11,460 years (two half-lives)?

**Part B: Doubling Time Analysis (15 minutes)**

4. The **Rule of 70** states: Doubling time ≈ 70 / (percentage growth rate)
   - Verify this rule for plastic production (8.4% growth)
   - Derive the exact formula: $T_{double} = \frac{\ln 2}{k}$
   - How accurate is the Rule of 70?

5. If plastic production continues at 8.4% annual growth:
...
6. A population of invasive carp in a lake follows $P(t) = 50 e^{0.15t}$, where $t$ is months.
...
7. Carbon dating: A fossil has 25% of its original C-14 remaining. How old is it?
...
1. **The Problem** (2 min): What real-world question were you answering?
2. **The Approach** (3 min): What mathematical model did you use and why?
3. **Key Results** (3 min): What did you find? Show at least one visualization.
4. **Implications** (2 min): What should decision-makers take away?
...
- $P(t) = P_0 \cdot b^t$ and $P(t) = P_0 e^{kt}$ are equivalent when $k = \ln(b)$
- Example: $2^t = e^{t \ln 2}$

**Logarithm rules for solving:**
- If $e^{kt} = C$, then $kt = \ln(C)$, so $t = \frac{\ln(C)}{k}$
...
![MRSA bacterial growth curve approaching shutdown threshold](handbook_images/W2A_bacterial_hospital.svg)
...
1. Write an exponential model $N(t) = N_0 \cdot 2^{t/T}$ where $T$ is the doubling time.
   - $N_0 = 800$ cells
   - $T = 25$ minutes
   - Calculate the population after 2 hours

2. Convert your model to the form $N(t) = N_0 e^{kt}$
   - Find the growth rate constant $k$ (hint: $e^k = 2^{1/25}$)
   - Verify both models give the same answer for $t = 120$ minutes

3. **Critical question:** How much time does the hospital have before the bacterial count reaches 1 million?
   - Solve $800 \cdot 2^{t/25} = 1,000,000$
   - Express your answer in hours and minutes

**Part B: Temperature Effects (15 minutes)**

4. Refrigeration slows bacterial growth. At 4°C, the doubling time increases to **180 minutes**.
   - What is the new growth rate constant $k_{cold}$?
...
5. The hospital has two options:
...
6. The "Rule of 70" states that doubling time ≈ 70/(percentage growth rate)
   - For a 25-minute doubling time, what percentage growth rate per minute does this imply?
   - Calculate the exact growth rate: $r = (e^k - 1) \times 100\%$ where $k = \ln(2)/25$
...
- [ ] Python plot showing $N(t)$ vs time with critical threshold marked
...
![Radioactive decay curves for Cs-137 and Sr-90](handbook_images/W2B_radioactive_fukushima.svg)
...
1. Write an exponential decay model $A(t) = A_0 \cdot (0.5)^{t/T_{1/2}}$ where $T_{1/2}$ is the half-life.
   - $A_0 = 500$ TBq
   - $T_{1/2} = 30.17$ years
   - Calculate the activity after 50 years

2. Convert your model to the form $A(t) = A_0 e^{-\lambda t}$
   - Find the decay constant $\lambda$ (hint: $e^{-\lambda} = 0.5^{1/30.17}$)
   - Verify both models give the same answer for $t = 50$ years

3. **Critical question:** How long until the waste can be reclassified as low-level (≤10 TBq)?
   - Solve $500 \cdot (0.5)^{t/30.17} = 10$
   - Express your answer in years

**Part B: Multiple Isotopes (15 minutes)**

4. The waste also contains **Strontium-90** with half-life **28.8 years** and initial activity **200 TBq**.
   ...
   - Solve: $500(0.5)^{t/30.17} = 200(0.5)^{t/28.8}$

**Part C: Storage Cost Analysis (10 minutes)**

5. High-level storage costs ¥50 million per year. Low-level storage costs ¥5 million per year.
...
6. **Carbon-14** has a half-life of 5,730 years and is used for dating ancient artifacts.
   - A fossil has 12.5% of its original C-14 remaining. How old is it?
   - Hint: 12.5% = (0.5)³, so it has gone through 3 half-lives
   - Verify using logarithms: solve $0.125 = (0.5)^{t/5730}$
...
![Schaefer growth curve with harvest lines and MSY point](handbook_images/W3_fishery_schaefer.svg)
...
Marine biologists have developed a **Schaefer model** for the sardine population:

$$G(S) = g \cdot S \cdot \left(1 - \frac{S}{K}\right)$$

where:
- $G(S)$ = annual population growth (tonnes/year)
- $S$ = current stock level (tonnes)
- $g = 0.4$ = intrinsic growth rate (per year)
- $K = 1500$ = carrying capacity (tonnes)

**Current situation:**
- Stock level: $S = 1200$ tonnes
- Industry harvest: $H = 120$ tonnes/year
...
1. Calculate the growth rate $G(S)$ at three stock levels:
   - $S = 400$ tonnes (heavily depleted)
   - $S = 750$ tonnes (half of carrying capacity)
   - $S = 1200$ tonnes (current level)

2. Show that growth is maximized when $S = K/2 = 750$ tonnes:
   - Expand the Schaefer equation: $G(S) = gS - \frac{g}{K}S^2$
   - This is a quadratic with vertex at $S = \frac{K}{2}$
   - Calculate the **Maximum Sustainable Yield (MSY)**

3. Plot $G(S)$ vs. $S$ for $S$ ranging from 0 to 1500 tonnes.

**Part B: Sustainability Analysis (20 minutes)**

4. A harvest is **sustainable** if the population can grow enough to replace what's removed:
   $$G(S) \geq H$$
   
   At the current stock level ($S = 1200$ tonnes):
   - Calculate $G(1200)$
   - Is the current harvest of 120 tonnes/year sustainable?

5. Find the **equilibrium stock levels** where $G(S) = H$:
   - Set up: $0.4S(1 - S/1500) = 120$
   - Expand: $0.4S - 0.0002667S^2 = 120$
   - Rearrange to standard form: $0.0002667S^2 - 0.4S + 120 = 0$
   - Solve using the quadratic formula
   - There should be two solutions—what do they represent?

6. The **lower equilibrium** is unstable (if stock drops below it, population collapses).
   The **upper equilibrium** is stable (population returns to it if perturbed).
   - Which equilibrium is safer for the fishery?
   - Is the current stock ($S = 1200$) above or below the safe equilibrium?

**Part C: Policy Recommendation (15 minutes)**

7. The fishing industry wants to increase harvest to 140 tonnes/year due to high demand.
   - Find the new equilibrium stock levels for $H = 140$
   - What is the minimum stock level needed to sustain this harvest?
   - Compare to the $H = 120$ scenario—is the risk acceptable?

8. Write a **brief recommendation** (4–5 sentences) to the Fisheries Authority:
...
- [ ] Completed Group Worksheet
- [ ] Python plot showing $G(S)$ curve with harvest levels marked
...
Focus on:
1. How the Schaefer model captures bounded growth
2. The concept of Maximum Sustainable Yield
3. Why there are two equilibrium points and what they mean
4. Your policy recommendation and reasoning
...
For the equation $0.0002667S^2 - 0.4S + 120 = 0$:
- $a = 0.0002667$
- $b = -0.4$
- $c = 120$
...
![Abalone growth parabola with current and proposed harvest levels](handbook_images/W3A_abalone.svg)

The WA Department of Fisheries manages a commercial abalone (*Haliotis roei*) fishery along the south coast. Population surveys and growth studies have led to the following Schaefer model:
...
- $G(S)$ = annual population growth (tonnes/year)
- $S$ = current stock level (tonnes)
- Intrinsic growth rate $g = 0.35$
- Carrying capacity $K = 1200$ tonnes

**Current situation:** Stock level is $S = 950$ tonnes. Industry wants to harvest **100 tonnes/year**.
...
1. Calculate growth $G(S)$ at three stock levels:
   - $S = 300$ tonnes (low stock)
   - $S = 600$ tonnes (mid stock) 
   - $S = 900$ tonnes (high stock)
   - Which stock level has the highest growth rate?

2. Show that growth is maximized at $S = K/2 = 600$ tonnes
   - Expand $G(S) = 0.35S(1 - S/1200)$ to standard form
   - Find the vertex of this parabola
   - What is the maximum sustainable yield (MSY)?

**Part B: Sustainability Analysis (20 minutes)**

3. A harvest is **sustainable** if $G(S) \geq H$ (growth ≥ harvest).
   - At current stock $S = 950$ tonnes, what is $G(950)$?
   - Is the proposed harvest of 100 tonnes/year sustainable?

4. Find the **two equilibrium points** where $G(S) = 100$:
   - Solve: $0.35S(1 - S/1200) = 100$
   - Hint: This is a quadratic equation. Use the quadratic formula.
   - Interpret: what do these two equilibrium points represent?

5. The lower equilibrium is **unstable** (population collapses if stock drops below it).
   The upper equilibrium is **stable** (population recovers to it).
   - Which equilibrium is safer for long-term sustainability?

**Part C: Policy Recommendation (15 minutes)**

6. The industry proposes increasing harvest to **120 tonnes/year** due to high market prices.
   - Find the new equilibrium points for $H = 120$
   - What is the minimum safe stock level?
   - Compare to the $H = 100$ scenario

7. Write a 4-5 sentence recommendation to the Fisheries Minister addressing:
...
8. Create a graph showing:
   - The growth curve $G(S)$ vs stock $S$
   - Horizontal lines at $H = 100$ and $H = 120$
...
![Rock lobster growth under current vs warming conditions](handbook_images/W3B_rock_lobster.svg)

Western Australia's rock lobster (*Panulirus cygnus*) fishery was severely overfished in the 1990s. After implementing catch limits, the fishery is recovering. Marine biologists have developed this Schaefer model:
...
- $G(S)$ = annual population growth (tonnes/year)
- $S$ = current stock level (tonnes)
- Intrinsic growth rate $g = 0.42$ (higher than abalone due to faster maturation)
- Carrying capacity $K = 800$ tonnes

**Current situation:** After recovery efforts, stock is now $S = 350$ tonnes. The fishing industry wants permission to harvest **70 tonnes/year**.
...
1. Calculate growth $G(S)$ at three stock levels:
   - $S = 200$ tonnes (recovering from collapse)
   - $S = 400$ tonnes (near optimal)
   - $S = 700$ tonnes (near carrying capacity)
   - At which stock does growth peak?

2. Prove that maximum growth occurs at $S^* = K/2 = 400$ tonnes
   - Write $G(S) = 0.42S - 0.000525S^2$ in standard form
   - Complete the square or use vertex formula
   - Calculate the maximum sustainable yield (MSY)

**Part B: Harvest Sustainability (20 minutes)**

3. At the current stock level $S = 350$ tonnes:
   - Calculate the natural growth rate $G(350)$
   - Is the proposed 70 tonnes/year harvest sustainable?
   - What fraction of growth would be harvested?

4. Find all stock levels where harvest equals growth: $G(S) = 70$
   - Set up: $0.42S(1 - S/800) = 70$
   - Multiply out: $0.42S - 0.000525S^2 = 70$
   - Rearrange: $0.000525S^2 - 0.42S + 70 = 0$
...
6. Ocean warming is expected to reduce carrying capacity to $K = 720$ tonnes by 2040.
   - What is the new MSY under warming scenario?
   - New model: $G(S) = 0.42S(1 - S/720)$
   - Can the fishery still sustain 70 tonnes/year harvest?

7. Write a brief (4-5 sentence) climate adaptation strategy for the fishery addressing:
...
8. Compare rock lobster to abalone (from Problem W3A):
   - Which has faster intrinsic growth ($g$)? 
   - Which has higher carrying capacity ($K$)?
...
![Chemical yield curve showing optimal temperature and derivative](handbook_images/W4_chemical_reaction.svg)

A pharmaceutical company is optimizing the production of a critical antibiotic. The yield of the drug depends on reaction temperature. Chemical engineers have developed an empirical model:

$$Y(T) = -0.05T^2 + 8T - 200$$

where:
- $Y(T)$ = yield (grams of product per batch)
- $T$ = reaction temperature (°C)
- Valid range: $50°C \leq T \leq 110°C$
...
1. Calculate the yield at three temperatures:
   - $T = 60°C$
   - $T = 80°C$
   - $T = 100°C$
...
2. Find the **derivative** $Y'(T) = \frac{dY}{dT}$:
   - Use the power rule: $(x^n)' = nx^{n-1}$
   - Interpret: What does $Y'(T)$ represent? Include units!

3. Calculate $Y'(70)$ and $Y'(90)$:
...
4. Find the **critical point** by solving $Y'(T) = 0$:
   - Set your derivative equal to zero
   - Solve for $T$
   - This is the candidate for maximum yield

5. Verify this is a maximum using the **second derivative test**:
   - Calculate $Y''(T)$
   - Evaluate $Y''(T_{critical})$
   - If $Y'' < 0$: maximum ✓
   - If $Y'' > 0$: minimum ✗

6. Is the optimal temperature within the safe range $[50, 110]°C$?
   - If YES: use it
   - If NO: check the boundary values

**Part C: Practical Constraints (15 minutes)**

7. The reaction vessel can only maintain temperature within ±3°C of the setpoint.
   - If the optimal temperature is $T_{opt}$, the actual temperature varies in $[T_{opt} - 3, T_{opt} + 3]$
   - Calculate the yield at both extremes
   - What's the range of possible yields?

8. A competing process uses $T = 70°C$ and claims "good enough" yields.
   - How much yield is lost compared to the optimum?
   - Express as a percentage

**Part D: Cost-Benefit Analysis (5 minutes)**

9. Higher temperatures require more energy. Energy cost is $\$0.50$ per degree above 50°C per batch.
   - Calculate the energy cost at the optimal temperature
   - If the drug sells for $\$5$ per gram, calculate net profit per batch at optimal temperature
   - Compare to net profit at $T = 70°C$
...
- [ ] Graph of $Y(T)$ with the critical point marked
...
Focus on:
1. The concept of derivative as rate of change
2. How optimization works (critical points, second derivative test)
3. The difference between mathematical optimum and practical optimum (constraints)
4. Your recommendation for the pharmaceutical company
...
**Power rule:**
- $(x^n)' = nx^{n-1}$
- $(c \cdot f(x))' = c \cdot f'(x)$ where $c$ is a constant
- $(f(x) + g(x))' = f'(x) + g'(x)$

**For $Y(T) = -0.05T^2 + 8T - 200$:**
- $Y'(T) = -0.1T + 8$
- $Y''(T) = -0.1$
...
![Pesticide concentration decay with effective zone and vulnerability window](handbook_images/W4A_pesticide.svg)

You are consulting for a wheat farm in the Wheatbelt. The farm uses a systemic pesticide that degrades over time after application. The concentration of active ingredient follows:

$$C(t) = 120 e^{-0.15t}$$

where:
- $C(t)$ = concentration (mg/L) in plant tissue
- $t$ = days after application
- Initial concentration: $C(0) = 120$ mg/L
...
1. Calculate concentration at $t = 0, 5, 10, 15, 20$ days
   - Create a table of values
   - How quickly is concentration dropping in the first week vs. third week?

2. Find the **instantaneous rate of change** $C'(t)$ using the derivative:
   - Use rule: $(e^{kt})' = k \cdot e^{kt}$
   - Simplify: $C'(t) = \frac{dC}{dt}$
   - Evaluate $C'(5)$ and interpret (include units!)

3. At what rate is the pesticide concentration decreasing on day 10?
   - Calculate $C'(10)$
   - Express as a percentage of $C(10)$: $\frac{C'(10)}{C(10)} \times 100\%$

**Part B: Effective Protection Window (20 minutes)**

4. Determine when concentration first drops below the effective threshold:
   - Solve: $120e^{-0.15t} = 30$
   - Take natural log of both sides
   - $t_{end} = ?$ days

5. How long is the "protection window" (concentration ≥ 30 mg/L)?
   - Protection duration = $t_{end} - 0$
   - Express in days and hours

6. If the farm wants to extend protection to 20 days, what initial concentration is needed?
   - Solve: $C_0 e^{-0.15 \times 20} = 30$
   - Would this exceed the phytotoxicity limit of 150 mg/L?

**Part C: Re-Application Strategy (15 minutes)**

7. The farmer wants continuous protection over a 30-day period (grain filling stage).
...
8. Calculate total pesticide used over 30 days:
...
9. The **half-life** is the time for concentration to halve.
   - Starting from 120 mg/L, when does it reach 60 mg/L?
   - Solve: $120e^{-0.15t} = 60$
   - Verify using the formula: $t_{1/2} = \frac{\ln(2)}{k}$ where $k = 0.15$

10. After two half-lives, what fraction of the original concentration remains?
...
- [ ] Graph showing $C(t)$ with protection window shaded
...
![Drug concentration profile with therapeutic window and dosing](handbook_images/W4B_pharmacokinetics.svg)

You are a pharmacologist designing a dosing protocol for metformin, a diabetes medication. After oral administration, blood concentration follows:

$$C(t) = 85(e^{-0.12t} - e^{-0.45t})$$

where:
- $C(t)$ = blood concentration (μg/mL)
- $t$ = hours after ingestion
...
1. Calculate concentration at $t = 0, 1, 2, 4, 8, 12$ hours
   - When does peak concentration occur? (look for the maximum in your table)
   - What is the peak value?

2. Find the **exact time** of peak concentration using calculus:
   - Take the derivative: $C'(t) = 85[-0.12e^{-0.12t} - (-0.45)e^{-0.45t}]$
   - Set $C'(t) = 0$ and solve for $t$
   - Hint: $-0.12e^{-0.12t} + 0.45e^{-0.45t} = 0$
   - Rearrange: $0.45e^{-0.45t} = 0.12e^{-0.12t}$
   - Divide both sides by $e^{-0.45t}$: $0.45 = 0.12e^{0.33t}$
   - Solve for $t$

3. Verify that the peak is safe:
   - Calculate $C(t_{peak})$
   - Is it below 100 μg/mL?

**Part B: Therapeutic Window (20 minutes)**

4. Determine when concentration drops below therapeutic threshold:
   - Solve numerically or graphically: $85(e^{-0.12t} - e^{-0.45t}) = 15$
   - Use Python to find the intersection point
   - How many hours of therapeutic coverage per dose?

5. Can patients safely use a 12-hour dosing interval?
   - Compare therapeutic duration to 12 hours
   - If not sufficient, what minimum concentration will they have at 12 hours?

6. To achieve 12-hour coverage with $C(12) = 15$ μg/mL, what dose multiplier is needed?
   - New model: $C(t) = D \cdot (e^{-0.12t} - e^{-0.45t})$ where $D$ is the dose factor
   - Solve: $D(e^{-0.12 \times 12} - e^{-0.45 \times 12}) = 15$
   - Does this keep peak concentration safe?

**Part C: Multiple Dosing Strategy (15 minutes)**

7. If a second dose is taken at $t = 12$ hours:
   - Residual from first dose: $C_1(12) = 85(e^{-0.12 \times 12} - e^{-0.45 \times 12})$
   - New dose contribution: $C_2(t) = 85(e^{-0.12(t-12)} - e^{-0.45(t-12)})$ for $t \geq 12$
   - Total concentration at $t = 12^+$: $C_{total}(12) = C_1(12) + 85$
   - Will this cause unsafe accumulation?

8. **Steady state:** After many doses, concentration oscillates between a minimum and maximum.
   - Estimate the steady-state maximum
   - Is it safe?

**Part D: Rate of Elimination (10 minutes)**

9. Calculate the rate of change of concentration:
   - $C'(t) = 85[-0.12e^{-0.12t} + 0.45e^{-0.45t}]$
   - Evaluate at $t = 2$ hours (around peak)
   - Evaluate at $t = 10$ hours (late elimination phase)
   - Which elimination rate is faster? Why does this make sense physiologically?

10. The two exponential terms represent:
    - $e^{-0.45t}$: rapid absorption and distribution
    - $e^{-0.12t}$: slower elimination
...
![Wildlife corridor layout and area optimisation curve](handbook_images/W5_wildlife_corridor.svg)
...
1. Let $x$ = width of corridor (meters) and $y$ = length (meters).

2. Write the **constraint equation** for perimeter:
   $$2x + 2y = 2000$$

3. Solve the constraint for $y$ in terms of $x$:
   $$y = ?$$

4. Write the **objective function** for area:
   $$A(x) = x \cdot y$$
   
   Substitute your expression for $y$ to get $A$ as a function of $x$ only.

5. What is the domain of $x$?
   - Mathematical: $x > 0$ (width must be positive)
   - Physical: $x \geq 200$ (minimum width) and $x \leq ?$ (ensure $y > 0$)

**Part B: Finding the Optimal Dimensions (25 minutes)**

6. Take the derivative: $A'(x) = ?$

7. Find the critical point by solving $A'(x) = 0$

8. Verify this is a maximum:
   - Calculate $A''(x)$
   - Evaluate $A''(x_{critical})$
   - Since $A'' < 0$, this is a maximum ✓

9. Is the critical point within the feasible range $[200, ?]$?
   - If YES: this is the optimal width
   - If NO: the optimum is at a boundary

10. Compare the area at:
    - The critical point (if feasible)
    - The minimum width boundary ($x = 200$)
    - The maximum width boundary (where $y = 200$, by symmetry)
    
    Which gives the largest area?

**Part C: Sensitivity Analysis (15 minutes)**

11. Budget constraints may reduce available fencing to 1800 m.
    - Repeat the optimization with perimeter = 1800 m
    - How much area is lost compared to the 2000 m scenario?

12. If the minimum width increases to 250 m (stricter ecological requirements):
    - Does this change the optimal design for 2000 m of fencing?
    - Calculate the new optimal area

**Part D: Recommendation (5 minutes)**

13. Write a brief (3-4 sentence) recommendation to the Department:
...
- [ ] Graph of $A(x)$ showing the critical point and constraints
...
Focus on:
1. How to handle constraints in optimization problems
2. The method: constraint substitution → single-variable optimization
3. Checking feasibility of mathematical solutions against physical constraints
4. Real-world trade-offs in your recommendation
...
1. Write constraint equation
2. Solve for one variable in terms of the other
3. Substitute into objective function → single-variable function
4. Take derivative and find critical points
5. Check feasibility and boundary values

**For this problem:**
- Constraint: $2x + 2y = 2000$ → $y = 1000 - x$
- Objective: $A(x) = x(1000 - x) = 1000x - x^2$
- Derivative: $A'(x) = 1000 - 2x$
- Critical point: $1000 - 2x = 0$ → $x = 500$
...
![Solar farm revenue optimisation curve](handbook_images/W5A_solar_farm.svg)
...
**Economic model:** Annual revenue (in thousands of dollars) is modeled as:

$$R(x) = -2x^2 + 180x - 1600$$

where $x$ is the average spacing between panel rows (in meters). This accounts for:
...
1. Find the derivative $R'(x)$:
   - Use power rule on each term
   - Interpret: $R'(x)$ represents the **rate of change** of revenue with respect to spacing

2. Find the critical point by solving $R'(x) = 0$:
   - Set your derivative equal to zero
   - Solve for $x$
   - This gives the **candidate** for optimal spacing

3. Verify this is a maximum using the **second derivative test**:
   - Calculate $R''(x)$
   - Evaluate $R''(x_{critical})$
   - If $R'' < 0$ → local maximum ✓
   - If $R'' > 0$ → local minimum ✗

4. Calculate the maximum revenue:
   - Substitute $x_{optimal}$ into $R(x)$
   - Express in dollars per year

**Part B: Constraint Boundaries (20 minutes)**

5. The critical point must lie within the feasible region $[8, 60]$. Check:
   - Is $8 \leq x_{optimal} \leq 60$? 
   - If YES: proceed to step 6
   - If NO: the optimum is at a boundary

6. Compare revenue at three points:
   - $R(x_{optimal})$ [if within constraints]
   - $R(8)$ [minimum spacing boundary]
   - $R(60)$ [maximum spacing boundary]
   - Which configuration yields highest revenue?

7. Find the **break-even points** where revenue = 0:
   - Solve: $-2x^2 + 180x - 1600 = 0$
   - Use quadratic formula: $x = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$
   - Interpret: below the lower break-even point, the farm loses money

**Part C: Sensitivity Analysis (15 minutes)**

8. Climate change projections suggest reduced solar irradiance will decrease revenue by 15%.
   - New model: $R_{climate}(x) = 0.85 \times R(x) = -1.7x^2 + 153x - 1360$
   - Find the new optimal spacing
   - How much does optimal revenue decrease?

9. To compensate for revenue loss, the company can install higher-efficiency panels (20% boost):
   - New model: $R_{upgrade}(x) = 1.20 \times R(x) = -2.4x^2 + 216x - 1920$
   - Find the new optimal spacing and revenue
   - Compare to the baseline scenario

**Part D: Economic Interpretation (5 minutes)**

10. Write a brief (4-5 sentence) recommendation to the company's CEO covering:
...
- [ ] Graph of $R(x)$ with critical point and boundaries marked
...
![Aquaculture profit as a function of stocking density](handbook_images/W5B_aquaculture.svg)
...
where:
- $P(d)$ = annual profit (thousands of dollars)
- $d$ = stocking density (fish per 100 m³)

**Constraints:**
- Minimum density: $d \geq 10$ (economic viability)
- Maximum density: $d \geq 80$ (animal welfare regulations)
...
1. Find the profit-maximizing stocking density:
   - Calculate $P'(d) = \frac{dP}{dd}$
   - Solve $P'(d) = 0$ for $d$
   - This gives the critical point

2. Confirm this is a maximum:
   - Calculate second derivative $P''(d)$
   - Since $P''(d) = -1 < 0$ everywhere, the function is concave down
   - Therefore the critical point is a global maximum ✓

3. Calculate maximum profit:
   - Substitute $d_{optimal}$ into $P(d)$
   - Express in dollars per year

4. Check feasibility:
   - Is $10 \leq d_{optimal} \leq 80$?
   - If NO: the constrained optimum is at a boundary

**Part B: Constrained Optimization (20 minutes)**

5. Compare profit at critical point vs. boundaries:
   - $P(d_{optimal})$ [if within $[10, 80]$]
   - $P(10)$ [minimum density]
   - $P(80)$ [maximum density]
   - Which is the true optimum given constraints?

6. Find the break-even density levels:
   - Solve: $-0.5d^2 + 55d - 800 = 0$
   - Multiply by -2: $d^2 - 110d + 1600 = 0$
   - Use quadratic formula
   - Interpret: farm loses money outside this range

7. Calculate the **marginal profit** at current operating density $d = 40$:
   - $P'(40) = ?$ dollars per fish (per 100 m³)
   - If $P'(40) > 0$: increasing density increases profit
   - If $P'(40) < 0$: decreasing density increases profit
   - What does this suggest?

**Part C: Comparative Scenarios (15 minutes)**

8. A disease outbreak reduces survival rates, decreasing the profit function to:
   $$P_{disease}(d) = -0.5d^2 + 55d - 1200$$
   - Find the new optimal density
   - How much does profit decrease at the optimum?
   - Does the farm still break even?

9. Installing aeration systems improves oxygen levels, changing the model to:
   $$P_{aeration}(d) = -0.4d^2 + 60d - 900$$
...
10. Write a brief (4-5 sentence) advisory memo to the farm manager:
...
![Carbon sequestration rate and cumulative capture over time](handbook_images/W6_carbon_sequestration.svg)
...
Forest ecologists have modeled the rate of carbon sequestration (absorption) for the proposed forest:

$$R(t) = 12e^{-0.05t}$$

where $R(t)$ is the sequestration rate in tonnes of CO₂ per hectare per year, and $t$ is years since planting.
...
1. Calculate the sequestration rate at $t = 0$, $t = 10$, and $t = 30$ years. What trend do you observe?

2. If the rate were *constant* at $R(0) = 12$ tonnes/ha/year, how much carbon would be sequestered in 40 years per hectare? Why will the actual total be less?

3. Explain in words why we need integration (not just multiplication) to find the total carbon sequestered.

**Part B: Antiderivatives (20 minutes)**

4. Find the indefinite integral (antiderivative):
   $$\int 12e^{-0.05t}\,dt$$
   
   Remember: $\int e^{kt}\,dt = \frac{1}{k}e^{kt} + C$

5. Verify your answer by differentiating. Does the derivative equal the original integrand?

6. Find the specific antiderivative $F(t)$ such that $F(0) = 0$. (This represents cumulative carbon sequestered since planting.)

**Part C: Project Valuation (20 minutes)**

7. Using your antiderivative, calculate the total carbon sequestered per hectare over 40 years:
   $$\text{Total} = F(40) - F(0)$$

8. Calculate total carbon sequestered across the entire 500-hectare project.

9. Calculate the total value of carbon credits generated over 40 years.

10. The government requires projects to deliver at least 100,000 tonnes of CO₂ sequestration to qualify for the offset program. Does this project qualify?

**Part D: Extension – Comparison (if time permits)**

11. An alternative project proposes planting a different species with sequestration rate:
    $$R_2(t) = 8(1 - e^{-0.1t})$$
...
1. The problem: We have a rate, we need a total
2. Why multiplication fails (rate isn't constant)
3. The antiderivative: reversing differentiation
4. The answer and its policy implications
...
For $\int 12e^{-0.05t}\,dt$:
- Factor out the constant 12
- Apply the rule with $k = -0.05$
- Result: $12 \times \frac{1}{-0.05}e^{-0.05t} + C = -240e^{-0.05t} + C$

**To find specific antiderivative with $F(0) = 0$:**
- Set $F(0) = -240e^0 + C = -240 + C = 0$
- Solve for $C$
...
![Soil carbon accumulation rate and cumulative storage](handbook_images/W6A_soil_carbon.svg)

A wheat farm in the Wheatbelt is transitioning to regenerative agriculture practices (no-till farming, cover crops, diverse rotations). Soil scientists have measured the rate of carbon accumulation in the topsoil:

$$R(t) = 15e^{-0.08t}$$

where:
- $R(t)$ = carbon sequestration rate (tonnes C per hectare per year)
- $t$ = years since transition began
...
1. Calculate the sequestration rate at $t = 0, 5, 10, 20$ years
   - What trend do you observe?
   - Why does the rate decrease?

2. If the rate were constant at $R(0) = 15$ tonnes/ha/year:
   - How much carbon would accumulate in 25 years per hectare?
   - Why will the actual total be less?

3. Explain in words why we need **integration** (not just multiplication) to find total carbon sequestered.
...
4. Find the indefinite integral:
   $$\int 15e^{-0.08t}\,dt$$
   - Remember: $\int e^{kt}\,dt = \frac{1}{k}e^{kt} + C$
   - Apply the rule with $k = -0.08$

5. Verify your answer by differentiating:
   - Does $\frac{d}{dt}[F(t)] = R(t)$?

6. Find the specific antiderivative $C(t)$ where $C(0) = 0$:
   - This represents **cumulative carbon** sequestered since $t=0$
   - Substitute $t=0$ and solve for the constant

**Part C: Carbon Credit Calculation (20 minutes)**

7. Calculate total carbon sequestered per hectare over 25 years:
   $$\text{Total} = C(25) - C(0)$$

8. Calculate total carbon across the entire 800-hectare farm.

9. Convert to CO₂-equivalent:
   - Multiply by 3.67 (molecular weight ratio)
   - This is the tradeable carbon credit quantity

10. Calculate total carbon credit revenue over 25 years:
    - (Total CO₂-e tonnes) × ($28 per tonne)
    - This is additional income from carbon farming

**Part D: Economic Comparison (5 minutes)**

11. Traditional farming on this land yields $400/ha/year profit.
    - Over 25 years: $400 × 800 × 25 = $8,000,000$
...
- [ ] Graph of $R(t)$ and $C(t)$ on separate subplots
...
![Groundwater infiltration rate and cumulative volume vs target](handbook_images/W6B_groundwater.svg)

The Water Corporation is implementing a managed aquifer recharge (MAR) scheme in the Perth Basin. During winter, excess stormwater is infiltrated into a confined aquifer. Engineers have modeled the infiltration rate:

$$I(t) = 20e^{-0.12t}$$

where:
- $I(t)$ = infiltration rate (megalitres per day)
- $t$ = days since infiltration began
...
1. Calculate infiltration rate at $t = 0, 30, 60, 90, 120$ days
   - Create a table
   - Describe the trend

2. If the rate were constant at $I(0) = 20$ ML/day:
   - How much water would infiltrate in 120 days?
   - Why will actual total be less?

3. Explain why integration is needed to find total volume infiltrated.

**Part B: Antiderivative Calculation (20 minutes)**

4. Find the indefinite integral:
   $$\int 20e^{-0.12t}\,dt$$
   - Use: $\int e^{kt}\,dt = \frac{1}{k}e^{kt} + C$
   - Simplify your answer

5. Verify by differentiating:
   - Check that $\frac{d}{dt}[V(t)] = I(t)$

6. Find the specific function $V(t)$ where $V(0) = 0$:
   - This represents **cumulative volume** infiltrated
   - Solve for the integration constant

**Part C: Water Storage Analysis (20 minutes)**

7. Calculate total volume infiltrated over 120-day winter period:
   $$V(120) = ?$$
   - Express in megalitres

8. Does the project meet its target of 1,500 ML?
   - Compare $V(120)$ to target
   - If not, how many additional days would be needed?

9. Calculate the economic value of recharged water:
   - Volume × $1,500 per ML
   - This is the benefit of the MAR scheme

10. Calculate average infiltration rate over the 120 days:
    $$\bar{I} = \frac{V(120) - V(0)}{120 - 0}$$
    - Compare to initial rate $I(0)$
    - Express as a percentage

**Part D: Seasonal Extension (5 minutes)**

11. To reach the 1,500 ML target, the Water Corporation considers extending the season.
    - Solve: $V(t) = 1500$ for $t$
...
![Supply and demand diagram with consumer and producer surplus](handbook_images/W7_market_welfare.svg)
...
**Demand function:** $Q_d = 500 - 4P$ (quantity in thousands of tonnes, price in \$/tonne)

**Supply function:** $Q_s = -100 + 2P$
...
1. Find the equilibrium price $P^*$ by setting $Q_d = Q_s$.

2. Find the equilibrium quantity $Q^*$.

3. Find the **maximum willingness to pay** (price at which $Q_d = 0$).

4. Find the **minimum acceptable price** (price at which $Q_s = 0$).
...
5. Rearrange $Q_d = 500 - 4P$ to get $P = D(Q)$ (inverse demand).

6. Rearrange $Q_s = -100 + 2P$ to get $P = S(Q)$ (inverse supply).

7. **Consumer Surplus formula:**
   $$CS = \int_0^{Q^*} D(Q)\,dQ - P^* \cdot Q^*$$
   
   Explain in words what each term represents. Why do we subtract $P^* \cdot Q^*$?

8. **Producer Surplus formula:**
...
9. Calculate Consumer Surplus using the integral formula. Show all steps.

10. Calculate Producer Surplus using the integral formula. Show all steps.

11. Calculate Total Welfare = CS + PS.

12. **Shortcut verification:** For linear demand and supply, the surplus areas are triangles:
...
13. The government proposes a price floor of \$120/tonne (above equilibrium).
    - At $P = 120$, what quantity do consumers demand?
    - At $P = 120$, what quantity do producers supply?
...
![Wine market equilibrium with price floor intervention](handbook_images/W7A_wine_market.svg)
...
**Demand:** $Q_d = 800 - 5P$ (quantity demanded, price in $/bottle)  
**Supply:** $Q_s = -200 + 3P$ (quantity supplied)
...
1. Find equilibrium price $P^*$ by setting $Q_d = Q_s$

2. Find equilibrium quantity $Q^*$

3. Calculate:
   - Maximum willingness to pay (price where $Q_d = 0$)
   - Minimum acceptable price for producers (price where $Q_s = 0$)

**Part B: Inverse Functions for Integration (15 minutes)**

4. Rearrange demand to get inverse demand: $P = D(Q)$
   - From $Q = 800 - 5P$, solve for $P$

5. Rearrange supply to get inverse supply: $P = S(Q)$
   - From $Q = -200 + 3P$, solve for $P$

6. Verify your inverse functions:
   - Substitute $Q^*$ into $D(Q)$ — do you get $P^*$?
   - Substitute $Q^*$ into $S(Q)$ — do you get $P^*$?
...
7. **Consumer Surplus** formula:
   ...
   - The integral $\int_0^{Q^*} D(Q)\,dQ$ = ?
   - The rectangle $P^* \cdot Q^*$ = ?
   - The difference = ?

8. Calculate Consumer Surplus:
   - Evaluate the definite integral
   - Subtract $P^* Q^*$
   - Express in dollars

9. **Producer Surplus** formula:
   $$PS = P^* \cdot Q^* - \int_0^{Q^*} S(Q)\,dQ$$
   
   Calculate Producer Surplus using integration.

10. **Verification using geometry:**
    - CS = $\frac{1}{2} \times Q^* \times (P_{max} - P^*)$
    - PS = $\frac{1}{2} \times Q^* \times (P^* - P_{min})$
...
12. The Association proposes a minimum price of $P_{floor} = 140$ (above equilibrium).
    - At this price, how much do consumers demand? $Q_d(140) = ?$
    - At this price, how much do producers supply? $Q_s(140) = ?$
    - What problem arises?
...
![Avocado market comparison: domestic only vs with exports](handbook_images/W7B_avocado_market.svg)
...
**Demand (domestic):** $Q_d = 600 - 4P$ (quantity in tonnes per week, price in $/kg)  
**Supply (domestic):** $Q_s = -120 + 2P$
...
1. Solve for equilibrium price $P^*$ and quantity $Q^*$

2. Find:
   - Maximum consumer willingness to pay
   - Minimum producer acceptable price

3. Sketch a rough supply-demand diagram

**Part B: Surplus Under Current Policy (25 minutes)**

4. Find inverse demand and supply functions:
   - $P = D(Q)$ from $Q_d = 600 - 4P$
   - $P = S(Q)$ from $Q_s = -120 + 2P$

5. Calculate **Consumer Surplus**:
   $$CS = \int_0^{Q^*} D(Q)\,dQ - P^* Q^*$$
   - Evaluate the integral $\int D(Q)\,dQ$
   - Apply limits [0, $Q^*$]
   - Subtract $P^* Q^*$
...
7. Verify using triangle formulas:
   - CS = $\frac{1}{2} Q^* (P_{max} - P^*)$
   - PS = $\frac{1}{2} Q^* (P^* - P_{min})$

8. Calculate Total Welfare and the ratio CS/PS

**Part C: Export Scenario (15 minutes)**

9. With export access, demand shifts to $Q_d^{new} = 720 - 4P$:
   - Find new equilibrium price $P^{new}$ and quantity $Q^{new}$
   - How much has price increased?

10. Calculate new Consumer Surplus and Producer Surplus:
    - Use the same integration approach
    - Compare to the baseline scenario

11. Who wins and loses from export expansion?
    - Change in CS = ?
    - Change in PS = ?
...
![Lotka-Volterra phase plane and population oscillations](handbook_images/W8_predator_prey.svg)

The Western Shield program aims to protect endangered numbats from feral cats. Ecologists have developed a predator-prey model:
...
1. Find the **trivial equilibrium** (both species extinct):
   - Set $N = 0, C = 0$

2. Find the **coexistence equilibrium**:
   - Set $\frac{dN}{dt} = 0$: $N(0.4 - 0.002C) = 0$
     - Either $N = 0$ or $C = ?$
   - Set $\frac{dC}{dt} = 0$: $C(-0.3 + 0.001N) = 0$
     - Either $C = 0$ or $N = ?$
   - **Coexistence:** $(N^*, C^*) = (?, ?)$

3. Interpret: What does this equilibrium mean for wildlife management?

**Part B: Phase Plane Analysis (20 minutes)**

4. **Nullclines** (curves where population growth = 0):
   - Numbat nullcline: $\frac{dN}{dt} = 0$ → $N = 0$ or $C = ?$
   - Cat nullcline: $\frac{dC}{dt} = 0$ → $C = 0$ or $N = ?$
...
6. Sketch the phase portrait showing how populations move in each region.

**Part C: Management Interventions (15 minutes)**

7. **Cat culling:** Remove 50 cats per year
   - Modified equation: $\frac{dC}{dt} = -0.3C + 0.001NC - 50$
   - Find the new equilibrium
   - How does numbat population change?

8. **Numbat translocation:** Add 20 numbats per year
   - Modified equation: $\frac{dN}{dt} = 0.4N - 0.002NC + 20$
   - Find the new equilibrium
   - Compare effectiveness to cat culling

9. Which intervention better supports numbat conservation? Why?
...
Focus on:
1. The Lotka-Volterra model structure
2. Equilibrium concepts (trivial vs. coexistence)
3. Phase plane analysis and population dynamics
...
- $N(0.4 - 0.002C) = 0$ means $N = 0$ OR $0.4 - 0.002C = 0$
...
![Detailed phase portrait with directional field and trajectory](handbook_images/W8A_cat_control.svg)

The Western Shield program aims to protect the endangered numbat (*Myrmecobius fasciatus*) from introduced predators, primarily feral cats. Ecologists have developed a simple predator-prey model:
...
where:
- $N$ = numbat population (number of individuals)
- $C$ = feral cat population (number of individuals)
- $t$ = time in years
...
1. Find the **trivial equilibrium** (extinction):
   - Set $N = 0, C = 0$
   - Is this a stable outcome?

2. Find the **coexistence equilibrium**:
   - Set both $\frac{dN}{dt} = 0$ and $\frac{dC}{dt} = 0$
   - From $\frac{dN}{dt} = 0$: $N(0.4 - 0.002C) = 0$
     - Either $N = 0$ (trivial) or $C = 200$
   - From $\frac{dC}{dt} = 0$: $C(-0.3 + 0.001N) = 0$
     - Either $C = 0$ or $N = 300$
   - **Coexistence equilibrium:** $(N^*, C^*) = (300, 200)$

3. Interpret the equilibrium:
   - 300 numbats and 200 cats can coexist indefinitely
   - If populations deviate, do they return to equilibrium or oscillate?

**Part B: Phase Plane Analysis (20 minutes)**

4. **Nullclines** (curves where growth rate = 0):
   - Numbat nullcline: where $\frac{dN}{dt} = 0$
     - Either $N = 0$ or $C = 200$ (horizontal line in phase plane)
   - Cat nullcline: where $\frac{dC}{dt} = 0$
     - Either $C = 0$ or $N = 300$ (vertical line in phase plane)
...
7. **Cat culling program:** Reduce cat population by 50 cats/year
   - Modified equation: $\frac{dC}{dt} = -0.3C + 0.001NC - 50$
   - New cat nullcline: $-0.3C + 0.001NC - 50 = 0$
   - Solve for new equilibrium: $C(-0.3 + 0.001N) = 50$
   - If $C = 200$: $200(-0.3 + 0.001N) = 50$
   - Solve for $N$: new numbat equilibrium

8. **Numbat translocation:** Add 20 numbats/year
   - Modified equation: $\frac{dN}{dt} = 0.4N - 0.002NC + 20$
   - Find new equilibrium

9. Which intervention is more effective for numbat conservation?
...
![Dingo-kangaroo dynamics: natural balance vs culling scenario](handbook_images/W8B_dingo_kangaroo.svg)

In arid Australia, dingoes (*Canis dingo*) are apex predators that regulate kangaroo populations. Removal of dingoes often leads to kangaroo overabundance and rangeland degradation. Ecologists model this system:
...
where:
- $K$ = kangaroo population (number of individuals)
- $D$ = dingo population (number of individuals)
- $t$ = time in years
...
1. Trivial equilibrium:
   - $(K, D) = (0, 0)$ — both species extinct
   - Is this realistic for arid Australia?

2. Coexistence equilibrium:
   - Set $\frac{dK}{dt} = 0$: $K(0.5 - 0.0015D) = 0$
     - Either $K = 0$ or $D = 333.33$ (round to 333)
   - Set $\frac{dD}{dt} = 0$: $D(-0.25 + 0.0008K) = 0$
     - Either $D = 0$ or $K = 312.5$ (round to 313)
   - **Equilibrium:** $(K^*, D^*) = (313, 333)$
...
4. Nullclines:
   - Kangaroo nullcline: $K = 0$ or $D = 333$
   - Dingo nullcline: $D = 0$ or $K = 313$
...
7. **Dingo culling:** Pastoralists remove 40 dingoes/year to protect livestock
   - New equation: $\frac{dD}{dt} = -0.25D + 0.0008KD - 40$
   - Find new equilibrium: $-0.25D + 0.0008KD - 40 = 0$
   - If $D = 333$: $-0.25(333) + 0.0008K(333) - 40 = 0$
   - Solve for new $K$ equilibrium

8. **Effect of culling:**
   - How much does kangaroo equilibrium increase?
   - What are the consequences for rangeland health?

9. **Alternative: Dingo protection**
   - No culling: equilibrium remains at $(313, 333)$
...
## IMPLEMENTATION NOTES FOR DEMONSTRATORS

**During the Working Week (e.g., Week 2):**
- ALL groups work on their assigned variation during lab time
- If time permits, groups can compare notes on the two variations
- Demonstrators should circulate and ensure both variations are being attempted

**For Presentation Week (e.g., Week 3):**
- Group 1 presents Variation A
- Group 2 presents Variation B
- The class benefits from seeing two distinct applications of the same mathematical concept
- Demonstrators should highlight commonalities and differences in the Q&A session

**Assessment Equity:**
- Both variations are designed to have equivalent difficulty
- Rubric criteria apply identically to both
- Students are assessed on their understanding of the mathematical concepts, not the specific context
...
![Diagnostic testing: population grid and PPV vs prevalence curve](handbook_images/W9_bayes_testing.svg)
...
2. Write out the given probabilities:
   - $P(T^+ | D)$ = sensitivity = ?
   - $P(T^- | D^c)$ = specificity = ?
   - $P(D)$ = prevalence = ?
...
3. Calculate the complementary probabilities:
   ...
   - $P(D^c)$ = probability of not having COVID = ?

**Part B: Applying Bayes' Theorem (25 minutes)**

4. We want to find $P(D | T^+)$: the probability of having COVID given a positive test.

   **Bayes' Theorem:**
   $$P(D | T^+) = \frac{P(T^+ | D) \cdot P(D)}{P(T^+)}$$

5. Calculate the **total probability** of testing positive:
   $$P(T^+) = P(T^+ | D) \cdot P(D) + P(T^+ | D^c) \cdot P(D^c)$$
   
   Substitute the values and calculate.

6. Now calculate $P(D | T^+)$ using Bayes' theorem.

7. **Interpret:** If someone tests positive, what's the chance they actually have COVID? Is it higher or lower than you expected?

**Part C: Sensitivity Analysis (15 minutes)**

8. How does prevalence affect the positive predictive value?
   - Recalculate $P(D | T^+)$ if prevalence increases to 10%
...
10. The government is deciding between two testing strategies:
...
11. Write a brief (3-4 sentence) recommendation about when rapid tests are most useful and when they should be confirmed with PCR tests.
...
- [ ] Graph showing $P(D | T^+)$ vs. prevalence
...
- Prior probability: $P(D)$ = what we know before the test
- Likelihood: $P(T^+ | D)$ = how good the test is
- Posterior probability: $P(D | T^+)$ = what we know after the test
...
![Mock exam structure: four question types at a glance](handbook_images/W10_mock_exam.svg)
...
c) At significance level $\alpha = 0.05$, should the company's claim be rejected? (4 marks)
...
A dairy farm produces milk at a cost of $C(x) = 2x^2 - 80x + 1200$ dollars, where $x$ is thousands of liters produced per week. Milk sells for \$1.20 per liter.

a) Write the profit function $P(x)$ (3 marks)

b) Find the production level that maximizes profit (5 marks)

c) What is the maximum weekly profit? (2 marks)

d) If fuel costs increase and the cost function becomes $C(x) = 2x^2 - 80x + 1500$, how does optimal production change? (2 marks)

---

**Question 3: Integration Application (12 marks)**

A solar panel's power output (in watts) varies throughout the day according to:
$$P(t) = 500\sin\left(\frac{\pi t}{12}\right)$$
where $t$ is hours after sunrise (0 ≤ t ≤ 12).

a) What is the peak power output and when does it occur? (3 marks)

b) Calculate the total energy produced during the day (energy = integral of power):
   $$E = \int_0^{12} P(t)\,dt$$
   (6 marks)
...
A water tank drains according to the differential equation:
$$\frac{dV}{dt} = -0.5\sqrt{V}$$
where $V$ is volume (liters) and $t$ is time (minutes). Initially, $V(0) = 100$ liters.

a) Separate variables and integrate to find $V(t)$ (6 marks)

b) How long until the tank is half empty? (3 marks)

c) How long until the tank is completely empty? (2 marks)

d) Sketch $V(t)$ vs. $t$ (1 mark)

---

#### DELIVERABLES

- [ ] Group solutions to all four questions
...
![Sinusoidal function with amplitude, period, and midline annotations](handbook_images/W11_sinusoidal.svg)
...
**Your group must create a problem involving a periodic phenomenon modeled by:**
...
**Required elements:**
1. **Real-world context** (temperature, tides, daylight hours, seasonal sales, heart rate, etc.)
2. **Four parameters determinable from data:**
   - $A$ = amplitude
   - $B$ = frequency (related to period $P = 2\pi/B$)
   - $C$ = phase shift
   - $D$ = vertical shift (midline)
3. **At least two questions:**
   - One asking for a value at a specific time
   - One asking when the function reaches a specific value
4. **Include enough data for the receiving group to determine all parameters**
...
Questions:
a) Find $A$, $B$, $C$, and $D$
b) What is the predicted temperature on April 10 (day 100)?
c) On what days of the year is the temperature 25°C?
...
![Linear programming feasible region with constraint lines](handbook_images/W12_linear_programming.svg)
...
1. Define decision variables:
   - Let $x$ = number of plant-based serves per week
   - Let $y$ = number of chicken serves per week

2. Write the **objective function** (what are we minimizing?):
   $$\text{Minimize } Z = 2.5x + 4y$$

3. Write the **constraint inequalities:**
   - Protein constraint: $18x + 30y \geq 350$
   - Calories constraint: $220x + 280y \geq 3500$
   - Iron constraint: $3x + y \geq 25$
   - Carbon constraint: $0.5x + 2.5y \leq 20$
   - Non-negativity: $x \geq 0, y \geq 0$

**Part B: Graphical Solution (25 minutes)**

4. Convert each constraint to an equation and find the boundary line:
   - Protein: $18x + 30y = 350$
     - When $x = 0$: $y = ?$
     - When $y = 0$: $x = ?$
   - Repeat for other constraints

5. Graph all constraints on the same axes. Shade the **feasible region** (the set of points satisfying ALL constraints).

6. Identify all **corner points** (vertices) of the feasible region by solving pairs of boundary equations simultaneously.
...
To find where protein and carbon constraints intersect:
- Protein: $18x + 30y = 350$
- Carbon: $0.5x + 2.5y = 20$
...
# 5. Group Worksheet Template

Your group should complete one worksheet per week and submit it via LMS.

**Grading:** Worksheets receive a **group mark**. Your **individual mark** is adjusted by your **peer-reviewed contribution score** (see Section 2.4).

---

## SCIE1500 GROUP WORKSHEET – WEEK ___
...
### SECTION 1: PROBLEM FORMULATION (20%)

**Briefly describe the real-world problem in your own words:**
...
**What mathematical model or technique is appropriate? Why?**

[Space for answer]

---

### SECTION 2: MATHEMATICAL SETUP (20%)

**Write out the key equations/functions:**

[Space for answer]

**Show initial calculations or setups:**

[Space for answer]

---

### SECTION 3: SOLUTION / IMPLEMENTATION (30%)

**Show your work step-by-step:**

[Space for detailed calculations]

**If using Python, attach code as an appendix**
...
**Include graphs/visualizations (attach as appendix or paste below):**

[Space for graphs]

---

### SECTION 5: INTERPRETATION (10%)

**What do your results mean for the decision-makers in this scenario?**

[Space for answer]

**What are the limitations or assumptions of your analysis?**

[Space for answer]

---

### SECTION 6: AI TOOL USAGE DECLARATION (5%)

**Did you use any AI tools (ChatGPT, Claude, Copilot, etc.)?**  
☐ Yes    ☐ No

**If yes, describe how you used them:**

[Space for answer]

**Attach screenshots or transcripts of AI interactions as appendix**
...
1. **Read carefully** – What is the question really asking?
2. **Identify knowns and unknowns** – What information do you have?
3. **Choose a method** – What technique from lectures applies?
4. **Set up mathematically** – Write equations before coding
5. **Solve/compute** – Do the math (by hand or Python)
6. **Check reasonableness** – Does your answer make sense?
7. **Interpret** – What does this mean in the real world?
...
- "Check my derivative: is $\frac{d}{dx}[x^2 e^x] = 2xe^x + x^2e^x$?"
...
**Document Version:** 2.0 (Combined with Variations)
