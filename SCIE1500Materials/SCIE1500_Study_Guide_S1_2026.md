...
*Interpretation: The lake approaches $C_{\mathrm{eq}} = 0.05$ mg/L exponentially, and reaches 90% of this long-run level after about $46$ hours. The mathematical structure here — separation of variables on a linear first-order ODE, giving an exponential approach to a steady state — is the same as the draining-tank ODE pattern you have seen in earlier weeks. Look for problems of the form $\frac{dy}{dt} = k(A - y)$: you will find the same setup and very similar algebra even though the physical context may differ.*
